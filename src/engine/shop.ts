...
function generateTraits(unitTypeId: UnitTypeId): PieceTrait[] {
  ...
  if (Math.random() < 0.5) {
    ...
    const applicableTraits = Object.values(TRAIT_DEFINITIONS).filter(t =>
      (t.applicableTo as readonly string[]).includes(unitTypeId)
    );
...
export function generateShopCards(playerLevel: number, pool: Map<UnitTypeId, number>): ShopCard[] {
...
export function createPieceFromCard(card: ShopCard, ownerId: string, matchId: string): Piece {
...
export function canMerge(pieces: Piece[]): boolean {
  ...
  return pieces.every(
    p => p.typeId === first.typeId && p.level === first.level && p.level < 3 // Max star level is 3
  );
}
