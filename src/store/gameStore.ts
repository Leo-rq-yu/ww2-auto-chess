  ...
  immer(set => ({
    ...
    buyCard: cardIndex =>
      set(state => {
        ...
        // Create piece
        const piece = createPieceFromCard(purchasedCard, state.currentUserId!, state.matchId!);
    ...
    movePieceOnBoard: (pieceId, newPosition) =>
      set(state => {
        state.board = movePiece(state.board, pieceId, newPosition);
      }),

    deployFromBench: (pieceId, position) =>
      set(state => {
    ...
    checkAndMerge: () =>
      set(state => {
        const allPieces = [...Object.values(state.board.pieces), ...state.bench].filter(
          p => p.ownerId === state.currentUserId
        );
    ...
    updatePhaseTimer: time =>
      set(state => {
        state.phaseTimeRemaining = time;
      }),

    // Battle
    setBattleState: (board, opponent) =>
      set(state => {
